...
class Enemy {
  constructor(px = 200.0, py = 200.0){
    this.x = px;
    this.y = py;
    this.points = 10.0;
    this.size = this.points * 10.0;
  }
  update(){}
  show(){
    rectMode(CENTER);
  ...
  toString(){
    return `${this.points}, ${this.x}, ${this.y}`
  }
}
...
let firingSpdFactor = 250.0;

function projectileTest() {
  if (millis() - lastSpawnTime > firingSpdFactor) {
    bullets.push(new Bullet(p.x, p.y, 0, -1));
  ...
  bullets = bullets.filter(b => {
    ...
    enemies.forEach(e => {
      if (b.hitsCircle(e.x, e.y, e.size / 2.0, e.size)) {
        ...
        if (e.hasDied()){
          const index = enemies.indexOf(e); // Finds the index of the first '20'

          if (index > -1) { // Only splice if element is found
            enemies.splice(index, 1); // Removes 1 element starting from 'index'
          }
        }
